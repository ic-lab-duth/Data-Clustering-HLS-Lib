#ifndef K_MEANS_BASELINE_H
#define K_MEANS_BASELINE_H
...
private:
  ...
  CID_TYPE get_nearest_center(P_TYPE pnt, DIST_TYPE &cost, C_TYPE clusters[K]) {
  ...
  void CCS_BLOCK(run) (P_TYPE points[N], C_TYPE clusters_centers[K]) {
    ...
    while ((iter <= MAX_ITER) && keep_going) {
      ...
      ASGN_TO_CLUST: for (int i=0; i<N; i++) {
        ...
        CID_TYPE new_cl_id = get_nearest_center(curr_pnt, cost, clusters_centers);
