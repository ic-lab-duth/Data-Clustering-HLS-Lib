...
#include <iostream>
    ...
    struct point_bs {
        ...
        point_bs() : id(-1), id_cluster(-1), x(0.0), y(0.0) {}
    };
    ...
    struct cluster_bs {
        ...
        cluster_bs() : id(-1), x(0.0), y(0.0), init_x(0.0), init_y(0.0) {}
    };


    class KMeans_basic {
      private:
        std::vector<cluster_bs> clusters;
        std::vector<point_bs> points;
      ...
      public:
        ...
        //DIONISIS
        std::vector<int> get_cluster_of_points();
        std::vector<std::pair<float, float>> get_cluster_centers();
