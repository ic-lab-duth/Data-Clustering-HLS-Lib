...
template<const int MAX_V, const int MIN_V, const int N, const int K, const int G>
...
*/
static const int PID_BITS = ac::nbits<N_POINTS>::val;  // define amount of bits needed to save point ids
static const int CID_BITS = ac::nbits<K_CLSTRS>::val;  // define amount of bits needed to save clusters ids
...
static const int ACC_BITS       = ACCUMULATOR_DATATYPE_INITIALIZATION<MAX_VALUE, MIN_VALUE, N_POINTS, K_CLSTRS, GRANULARITY>::bits;
static const int ACC_I_BITS     = ACCUMULATOR_DATATYPE_INITIALIZATION<MAX_VALUE, MIN_VALUE, N_POINTS, K_CLSTRS, GRANULARITY>::i_bits;
static const int ACC_SIGNEDNESS = ACCUMULATOR_DATATYPE_INITIALIZATION<MAX_VALUE, MIN_VALUE, N_POINTS, K_CLSTRS, GRANULARITY>::sign;


/*
*   MAIN DATATYPES DEFINITION
*/
typedef ac_int<PID_BITS, false>                                              PID_TYPE;     // Define size of data type for poits IDs
typedef ac_int<CID_BITS, false>                                              CID_TYPE;     // Define size of data type for cluster IDs
typedef ac_fixed<PID_BITS, PID_BITS, COORD_SIGNEDNESS, AC_TRN, AC_SAT>       COUNTER_TYPE; // datatype for the counter variables (used to count the amount of points in each cluster)
